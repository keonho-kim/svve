...
- 고정된 단계 순서로 SVVE를 정의
- PRF 확장을 필수 단계로 명시
- 고정 수치와 종료 조건을 명확히 규정

## 2. 문제 정의

SVVE는 "이미 임베딩이 구축된 단일 VDB 인덱스"를 전제로 한다.
...
### 4.2 Step 2: 세그먼트 병렬 검색

각 세그먼트 질의를 동일 VDB 인덱스에 병렬 전송한다.
...
반복은 `|F| >= K`가 될 때까지 지속한다. 즉, 최종 Top-K 목표 개수를 채울 때까지 추가 검색/후보 재정렬 루프를 고정 수행한다.

## 5. 이론 기반 파이프라인 요약

1. 쿼리 입력 검증(`numpy.float32`, 1D)
2. 4세그먼트 질의 생성
3. 세그먼트별 Top-100 병렬 검색
4. 3/2/1 투표 규칙으로 노이즈 제거 및 생존 5개 선택
5. PRF 보정 쿼리 생성(`q* = 0.7q + 0.3c`)
6. 추가 검색/후보 재정렬을 `K` 충족까지 반복
...
```python
engine = SearchEngine(index_root="...")
