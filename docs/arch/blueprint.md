...
본 프로젝트는 이미 구축된 Vector Database(VDB)를 전제로, 검색 시점의 처리 방식을 최적화해 응답 성능과 검색 품질을 높이는 Python 패키지(`svve_core`)를 목표로 한다.
...
VDB 호출은 어댑터 구현 방식에 따라 두 경로 중 하나를 택한다.

- 동기 SDK 기반: Rust 스레드에서 병렬 요청 후 결과 병합
- 비동기 SDK 기반: Python 이벤트 루프 또는 별도 어댑터에서 동시 요청 처리

핵심은 "I/O 동시성"과 "CPU 집계 병렬성"을 분리해 관리하는 것이다.
...
1. **서버 기동 및 상태 초기화**
   `SearchEngine`을 생성하고 VDB 연결 정보(엔드포인트, 컬렉션/인덱스명)를 바인딩한다.
2. **쿼리 수신 및 검증 (Python)**
   외부 임베딩 모델이 생성한 `numpy.float32` 1D 벡터를 입력받아 검증한다.
3. **FFI 진입 (PyO3)**
   NumPy 데이터를 Rust로 전달하고 검색 파이프라인을 시작한다.
4. **분할 질의 생성 및 병렬 검색 (고정)**
   쿼리를 4개 세그먼트 질의로 고정 분할하고(`N=4`), 세그먼트별 Top-100(`k_seg=100`)을 동일 VDB에 병렬 요청한다.
...
7. **Top-K 충족 반복 (고정)**
   추가 검색과 후보 재정렬을 반복해 최종 결과 개수가 요청 `top_k`를 채울 때까지 수행한다.
8. **최종 Top-K 반환**
...
- 알고리즘 종료 기준: 최종 후보가 요청 `top_k`를 충족할 때
